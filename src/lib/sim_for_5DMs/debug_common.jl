...
function process_pcm_data_common(input_file1::String, input_file2::String, output_file::String, n::Int, k::Int)
    # CSVファイルを読み込む
    df1 = CSV.read(input_file1, DataFrame)
    df2 = CSV.read(input_file2, DataFrame)

    # `entani`論文のCommonGroundが存在、解の非唯一性考慮のCommonGroundが存在しないTrial番号を抽出
    selected_trials = df1[(df1[:, :entani論文のCommonGround] .!= "Error") .& 
                      (df1[:, :解の非唯一性考慮のCommonGround] .== "Error"), :PCM_Identifiers]
    ...
    for i in unique_trial_numbers
        ...
        for m =1:k
            ...
            if(m!=k)
                ...
                commonbefore = solveCommonGroundLP(A)
                ...
                tcommon = solvetCommonGroundLP2(A)
                if tcommon === nothing
