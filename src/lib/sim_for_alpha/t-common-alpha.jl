...
LPResult_t_3_CommonGround = @NamedTuple{
    # 区間重みベクトル
...
function solvetCommonGroundLP_alpha(matrices::Vector{Matrix{T}})::Union{LPResult_t_3_CommonGround, Nothing} where {T <: Real}
    ε = 1e-8 # << 1
    ...
    try
        ...
        for k = 1:l
            ...
            for i = 1:n
                ...
                @constraint(model, wᵢᴸ_tcommon_center_1 ≥ (1+x)/2*ŵₖᵢᴸ_tcommon_center_1 + (1-x)/2*ŵₖᵢᵁ_tcommon_center_1)
                @constraint(model, wᵢᵁ_tcommon_center_1 ≥ wᵢᴸ_tcommon_center_1)
                @constraint(model, (1-x)/2*ŵₖᵢᴸ_tcommon_center_1 + (1+x)/2*ŵₖᵢᵁ_tcommon_center_1 ≥ wᵢᵁ_tcommon_center_1)
            end
        end
