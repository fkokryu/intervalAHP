...
function convert_weights_to_matrices(df::DataFrame)
    ...
    for i in 1:n_rows
        ...
        # entani論文の重み
        entani_wᴸ[i] = parse_weight_vector(row.entani論文のPartialIncorporation_wᴸ)
        entani_wᵁ[i] = parse_weight_vector(row.entani論文のPartialIncorporation_wᵁ)
        
        # 解の非唯一性考慮の重み
        nonunique_wᴸ[i] = parse_weight_vector(row.解の非唯一性考慮のPartialIncorporation_wᴸ)
        nonunique_wᵁ[i] = parse_weight_vector(row.解の非唯一性考慮のPartialIncorporation_wᵁ)
        
        # 各DMの重みを取得
        entani_wᴸ_dm1 = parse_weight_vector(row.entani論文のPartialIncorporation_wᴸ_1)
        entani_wᵁ_dm1 = parse_weight_vector(row.entani論文のPartialIncorporation_wᵁ_1)
        entani_wᴸ_dm2 = parse_weight_vector(row.entani論文のPartialIncorporation_wᴸ_2)
        entani_wᵁ_dm2 = parse_weight_vector(row.entani論文のPartialIncorporation_wᵁ_2)
        entani_wᴸ_dm3 = parse_weight_vector(row.entani論文のPartialIncorporation_wᴸ_3)
        entani_wᵁ_dm3 = parse_weight_vector(row.entani論文のPartialIncorporation_wᵁ_3)
        
        nonunique_wᴸ_dm1 = parse_weight_vector(row.解の非唯一性考慮のPartialIncorporation_wᴸ_1)
        nonunique_wᵁ_dm1 = parse_weight_vector(row.解の非唯一性考慮のPartialIncorporation_wᵁ_1)
        nonunique_wᴸ_dm2 = parse_weight_vector(row.解の非唯一性考慮のPartialIncorporation_wᴸ_2)
        nonunique_wᵁ_dm2 = parse_weight_vector(row.解の非唯一性考慮のPartialIncorporation_wᵁ_2)
        nonunique_wᴸ_dm3 = parse_weight_vector(row.解の非唯一性考慮のPartialIncorporation_wᴸ_3)
        nonunique_wᵁ_dm3 = parse_weight_vector(row.解の非唯一性考慮のPartialIncorporation_wᵁ_3)
...
function process_and_save_interval_pcms_partial(input_file::String, output_file::String)
    # CSVファイルを読み込む
    df = CSV.read(input_file, DataFrame)
    
    # "entani論文のPartialIncorporation_wᴸ" 列に数値が存在する行の "Trial" 列を選択
    valid_rows = .!ismissing.(df[:, :"entani論文のPartialIncorporation_wᴸ"]) .&
                .!(df[:, :"entani論文のPartialIncorporation_wᴸ"] .== "E")
...
function main()
    # CSVファイルのパス
    input_file = "weights_partial_5_3_0.65.csv"
    output_file = "pcm_partial_5_3_0.65.csv"
    
    # 区間PCMを計算して保存
    process_and_save_interval_pcms_partial(input_file, output_file)
    
    # 結果を表示する例
    # 出力されたCSVファイルから特定のトライアルのPCMを表示
    # display_filtered_interval_pcm(output_file, 1)  # Trial 1のすべての区間PCM
end