...
function write_interval_info_per_trial(input_file1::String, output_dir::String)
    using DelimitedFiles
    mkpath(output_dir)

    df = CSV.read(input_file1, DataFrame)

    selected_trials = df[.!ismissing.(df[:, :"entani論文のPerfectIncorporation_wᴸ"]) .&
                         .!(df[:, :"entani論文のPerfectIncorporation_wᴸ"] .== "E"), :"Trial"]
    unique_trial_numbers = unique(selected_trials)
    ...
    for i in 1:length(unique_trial_numbers)
        ...
        for dm in 1:3
            ŵᴸ = ŵᴸ_perfect[i][:, dm]
            ŵᵁ = ŵᵁ_perfect[i][:, dm]
            Ĥᴸ = interval_pcm_from_weights(ŵᴸ, ŵᴸ)
            Ĥᵁ = interval_pcm_from_weights(ŵᵁ, ŵᵁ)

            open(joinpath(trial_dir, "dm$(dm)_weights.txt"), "w") do io
                println(io, "DM$(dm) Lower Weights: ", ŵᴸ)
                println(io, "DM$(dm) Upper Weights: ", ŵᵁ)
            end

            open(joinpath(trial_dir, "dm$(dm)_pcm.txt"), "w") do io
                println(io, "DM$(dm) Lower PCM:")
                writedlm(io, Ĥᴸ)
                println(io, "\nDM$(dm) Upper PCM:")
                writedlm(io, Ĥᵁ)
            end
